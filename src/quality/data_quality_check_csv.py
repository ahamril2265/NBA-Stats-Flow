...
logger = get_logger("DATA_QUALITY_CSV")
...
def check_data_quality(snapshot_date: str):
    ...
    if "data_quality" not in config["base"]:
        raise KeyError("data_quality config missing in base.yaml")
    ...
    logger.info(f"START | CSV data quality check for {snapshot_date}")

    if not os.path.exists(silver_csv_path):
        raise FileNotFoundError("Silver CSV data not found")
    ...
    if metrics["status"] == "FAILED":
        raise ValueError("CSV data quality checks failed")

    logger.info(
        f"END | CSV data quality check completed with status {metrics['status']}"
    )
